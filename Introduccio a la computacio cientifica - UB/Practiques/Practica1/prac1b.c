...
int gausspivot(int n, double **A, double *v, double tol){
	...
	/*s'assigna un tamany de memoria per la variabla x*/
	...
	for(r=0; r<n-1; r++){
		...
		/*Fa els calculs i en el cas de la matriu A, posa 0's per sota de la diagonal.*/
		for(i = r+1; i<n; i++){
			/*comprova que el primer valor de la diagonal sigui mes gran que la tolerancia, si no es el cas retorna un 1.*/
			if(fabs(A[r][r])<tol){
				return 1;
			/*per altre banda, es busca el valor de m, que  es la posicio on es vol posar un 0, dividit per el valor de la diagonal que estigui per sobre*/
			}else{
				m = A[i][r]/A[r][r];
				A[i][r] = 0.f;
				for(j=r+1; j<n; j++){
	...
	/*es crida la funcio resoltrisup per trobar les x's, de Ax = b, on A es una matriu amb 0's per sota de la diagonal*/
...
int main(void) {
    int n, i, j, res;
    double **A, *b, tol, *y, *v;
    printf("Doeneu les dimensions de les matriu quadrada A, (n,n) = \n");
    scanf("%d", &n);
