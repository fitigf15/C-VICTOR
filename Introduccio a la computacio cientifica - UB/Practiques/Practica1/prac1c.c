...
int inversaA(int n, double **A, double **B, double tol){
	...
	for(i=0; i<n; i++){
		/*es copian tans cops com n, les columnes de B en b, per trobar les x, que a la vegada son els valors de la columna de la matriu inversa*/
		for(j=0; j<n; j++){
			b[j] = B[j][i];
		}
		...
		if(res == 1){
			...
			/*com a la sortida s'utilitza B, es copian a B els resultats que s'obtinguin de resoltrisup*/
			for(j=0; j<n; j++){
				B[j][i] = x[j];
			}
		}
...
int main(void) {
    ...
    /*S'omple la matiru B, amb els valor de la matriu identitat, posan 1 a la diagonal i a la resta 0*/
	for(i = 0; i<n; i++){
		for(j = 0; j<n; j++){
			if(i == j){
				B[i][j] = 1;
